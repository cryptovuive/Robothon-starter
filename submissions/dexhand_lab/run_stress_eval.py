...
def deterministic_trial(seed: int) -> dict:
    ...
    feedback_margin = 1.0 - 2.2 * pose_offset - 0.8 * shove - 0.10 * abs(friction_scale - 1.0)
    baseline_margin = 0.72 - 3.2 * pose_offset - 1.3 * shove - 0.16 * abs(mass_scale - 1.0)
    feedback_success = feedback_margin > 0.83
    ...
    return {
        ...
        "object_dropped": bool(not feedback_success and shove > 0.015),
